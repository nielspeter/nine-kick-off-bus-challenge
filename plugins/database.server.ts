  ...
  try {
    ...
    try {
      ...
      if (fs.existsSync(seedSqlPath)) {
          ...
          if (statement.trim()) {
            try {
              ...
              if (!error.message.includes('already exists') && !error.message.includes('duplicate key')) {
                console.warn('⚠️ SQL statement failed:', statement.substring(0, 100) + '...', error.message)
              }
            }
          }
        }
        ...
        // Verify seeding
        const [newUserCount, newTaskCount] = await Promise.all([
          User.count(),
          Task.count()
        ])
  ...
  const fallbackTasks = [
    {
      ...
      category: "Test",
      description: "Use AI to design the most creative test cases for a dating app specifically for pets.",
      estimatedTime: 25,
      difficulty: 2
    },
    {
